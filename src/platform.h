...
#define _EVERY_N_HELPER(token, interval_ms)                                    \
  for (static unsigned long _last_##token = 0;                                 \
       ((millis() - _last_##token) >= (interval_ms))                           \
           ? (_last_##token = millis(), true)                                  \
           : false;)
...
// Grid configuration is now in main.cpp as runtime variables
// to support multiple layout presets (see main.cpp for LayoutType enum)
extern int GRID_WIDTH;
extern int GRID_HEIGHT;
extern float LED_SPACING_H;
extern float LED_SPACING_V;
...
inline uint16_t beatsin16(uint16_t bpm, uint16_t lowest = 0,
  ...
  return beatsin8(bpm, lowest >> 8, highest >> 8, timebase, phase_offset >> 8)
         << 8;
}
